    ...
    public function index() {
        ...
        if (!empty($this->_templateData['shipping_method']['code'])) {
            ...
            $this->simplecheckout->addError('shipping');
				...
				$this->_templateData['pickpoint_url'] = $this->url->link('extension/shipping/pickpoint/setprice', '', 'SSL');


				$code = '';
				if (isset($this->request->post['shipping_method'])) $code = $this->request->post['shipping_method'];
				//if ($code == '' && isset($this->session->data['shipping_method']['code'])) $code = $this->session->data['shipping_method']['code'];

				if (strstr($code, 'pickpoint')!=false)
				{
					if (!isset($this->session->data['pickpoint_title'])) 
					{
				            $this->_templateData['has_error_shipping'] = true;
					 	$this->_templateData['error_shipping']     = $this->language->get('error_pickpoint');
