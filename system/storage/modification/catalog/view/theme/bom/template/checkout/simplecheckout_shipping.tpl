    ...
    <div class="alert alert-danger simplecheckout-warning-block" <?php echo $display_error && $has_error_shipping ? '' : 'style="display:none"' ?>><?php echo $error_shipping ?></div>
			...
			<script type="text/javascript" src="https://pickpoint.ru/select/postamat.js"></script>
				...
				$(document).ready(function(){
					var length = document.getElementsByName('pickpoint_shipping_div').length;
	
					for (var i=0;i<length;i++)
					{
						var pickpoint_html = document.getElementsByName('pickpoint_shipping_div')[i].innerHTML;
	
						if (pickpoint_html.indexOf('my_function') == -1)
						document.getElementsByName('pickpoint_shipping_div')[i].innerHTML += '<a style="text-decoration:underline;" href="#" onclick="PickPoint.open(my_function); return false;">  выберите постамат</a>';
					}
				});
				function my_function(result){	
						...
						//}      	     
							...
							type: 'post',
							data: {pickpoint_terminal_id:result['id'], pickpoint_terminal_address:result['address'], pickpoint_terminal_region:result['region'], pickpoint_terminal_cityid:result['cityid']},
							dataType: 'html',
