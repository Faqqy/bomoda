...
class OrderGrastin
{
    ...
    //Индекс
    public function set_zipcode($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['zipcode'] = $arg;
        }
        return $this;
    }

    //Регион
    public function set_region($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['region'] = $arg;
        }
        return $this;
    }

    //Район
    public function set_district($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['district'] = $arg;
        }
        return $this;
    }

    //Город
    public function set_city($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['city'] = $arg;
        }
    ...
    public function set_weight($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['weight'] = $arg;
        }
        return $this;
    }

    //ID пункта самовывоза (заполняется в случае доставки заказа через пункт самовывоза)
    public function set_pickup($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['pickup'] = $arg;
        }
        return $this;
    }

    //ID индекса города доставки (заполняется в случае доставки заказа курьером)
    public function set_postcode($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['postcode'] = $arg;
        }
        return $this;
    }

    //Стоимость доставки для покупателя
    public function set_costdelivery($arg)
    {
        if (!empty($arg) or !is_null($arg)) {
            $this->args['costdelivery'] = $arg;
        }
        return $this;
    }
}
