...
include_once DIR_SYSTEM . 'library/ll/grastin/src/GrastinException.php';
...
class Grastin
{
    ...
    private function sendRequest($xml)
    {
        ...
        if (!empty($parse_responce_xml['Error'])) {
            throw new GrastinException("Grastin Error: " . json_encode($parse_responce_xml['Error']));
        }
    ...
    public function selfpickup($city = '')
    {
        $xml = "<File><API>" . $this->key . "</API><Method>selfpickup</Method><City>" . $city . "</City></File>";
        $data = $this->sendRequest($xml);

        if (isset($data['Selfpickup'])) {
            return $data['Selfpickup'];
        }
    }

    //Получение списка пунктов приема заказов
    public function ordertakepoint()
    {
        $xml = "<File><API>" . $this->key . "</API><Method>ordertakepoint</Method></File>";
        $data = $this->sendRequest($xml);
        return $data['OrderTakePoint'];
    }
    ...
    public function boxberryselfpickup($city = '')
    {
        $xml = "<File><API>" . $this->key . "</API><Method>boxberryselfpickup</Method><City>" . $city . "</City></File>";
        $data = $this->sendRequest($xml);

        if (isset($data['SelfpickupBoxberry'])) {
            return $data['SelfpickupBoxberry'];
        }
    }

    //Получение списка доступных индексов Boxberry
    public function boxberrypostcode($city = '')
    {
        $xml = "<File><API>" . $this->key . "</API><Method>boxberrypostcode</Method><City>" . $city . "</City></File>";
        $data = $this->sendRequest($xml);

        if (isset($data['PostcodeBoxberry'])) {
            return $data['PostcodeBoxberry'];
        }
    }

    //Получение списка пунктов самовывоза Hermes
    public function hermesselfpickup($city = '')
    {
        $xml = "<File><API>" . $this->key . "</API><Method>hermesselfpickup</Method><City>" . $city . "</City></File>";
        $data = $this->sendRequest($xml);

        if (isset($data['SelfpickupHermes'])) {
            return $data['SelfpickupHermes'];
        }
    }

    //Получение списка партнерских пунктов самовывоза
    public function partnerselfpickup($city = '')
    {
        $xml = "<File><API>" . $this->key . "</API><Method>partnerselfpickup</Method><City>" . $city . "</City></File>";
        $data = $this->sendRequest($xml);

        if (isset($data['SelfpickupPartner'])) {
            return $data['SelfpickupPartner'];
        }
    }
    ...
    public function newordermail(array $og)
    {
        $orders_xml = '';
        foreach ($og as $item) {
            $orders_xml .= $item->convertToXML();
        }
        $xml = "<File><API>" . $this->key . "</API><Method>newordermail</Method><Orders>" . $orders_xml . "</Orders></File>";
    ...
    public function neworderboxberry(array $og)
    {
        $orders_xml = '';
        foreach ($og as $item) {
            $orders_xml .= $item->convertToXML();
        }
        $xml = "<File><API>" . $this->key . "</API><Method>neworderboxberry</Method><Orders>" . $orders_xml . "</Orders></File>";
    ...
    public function neworderhermes(array $og)
    {
        $orders_xml = '';
        foreach ($og as $item) {
            $orders_xml .= $item->convertToXML();
        }
        $xml = "<File><API>" . $this->key . "</API><Method>neworderhermes</Method><Orders>" . $orders_xml . "</Orders></File>";
    ...
    public function neworderpartner(array $og)
    {
        $orders_xml = '';
        foreach ($og as $item) {
            $orders_xml .= $item->convertToXML();
        }
        $xml = "<File><API>" . $this->key . "</API><Method>neworderpartner</Method><Orders>" . $orders_xml . "</Orders></File>";
    ...
    public function orderinformationbynumber(array $og)
    {
        //Формируем заказы
        $orders_xml = '<Orders>';
        foreach ($og as $item) {
    ...
    public function CalcShipingCost(array $orders){
        ...
        foreach($orders as $order){
            ...
            $order['selfpickup'] = $this->parse_parameters_calc_shiping($order,'selfpickup',false);
    ...
    {
        $xml = "<File><API>" . $this->key . "</API><Method>orderlist</Method><Status>" . $state . "</Status></File>";
        $data = $this->sendRequest($xml);
