...
function ocmCheckMods() {
  ...
  if (version_compare(VERSION, '2', '>=')) {
    $files = glob(OCM_DIR . 'extension/*/*.ocmod.xml');
  } else {
    $files = glob(OCM_DIR . 'extension/*/*.vqmod.xml');
  }

  if ($files) {
    foreach ($files as $file) {
      $code = basename(dirname($file));
      if ($code == 'ocmodify' || OCModify::$instance->config->get($code . '_status')) {
...
function ocmCheckModCache() {
  $error = false;
  if (version_compare(VERSION, '2', '>=')) {
    if (!glob(OCM_DIR_MOD . '*', GLOB_ONLYDIR)) {
      $error = true;
    } else {
      foreach ((array)glob(OCM_DIR_XML . '*.ocmod.xml') as $file) {
        if (filemtime($file) > filemtime(OCM_DIR_MOD . 'system')) {
          $error = true;
          break;
        }
      }
    ...
    if (!glob(OCM_DIR_MOD . '*.cache')) {
      $error = true;
    } else {
      foreach ((array)glob(OCM_DIR_MOD . '*.cache') as $cache) {
...
function ocmRefreshMods() {
  ...
  if (version_compare(VERSION, '2', '>=')) {
    ...
    if (OCM_ADMIN_FOLDER && ocmIsLogged(OCModify::$instance)) {
      ...
      if (!$status && isset($_GET['debug'])) {
        trigger_error('Error: Could not automatically refresh modification cache!');
      }
    }
  } elseif (is_dir(OCM_DIR_MOD)) {
    $status = ocmVQmodFlush();
  }
...
function ocmVQmodFlush() {
  // Clean
  foreach ((array)glob(OCM_DIR_MOD . '{*.cache}', GLOB_BRACE) as $file) {
    unlink($file);
  }

  // Check
  $caches = glob(OCM_DIR_MOD . '{*.cache}', GLOB_BRACE);
...
function ocmCheckInstall($extension_code) {
  ...
  if (($install && !$query->num_rows) || (!$install && $query->num_rows)) {
    ...
    if (is_file(OCM_DIR . 'extension/' . $extension_code . '/config.php')) {
      include_once(OCM_DIR . 'extension/' . $extension_code . '/config.php');
    }
