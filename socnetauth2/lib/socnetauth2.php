...
Class SocAuth extends DB
{
	...
	public function sendConfirmEmail($data)
	{
		...
		{
			include(DIR_LANGUAGE.'en-gb/account/socnetauth2.php');
		}
		else
		{
			include(DIR_LANGUAGE.'ru-ru/account/socnetauth2.php');
		}
	...
	{
		$ses = $_SESSION;
		foreach ($ses as $skey=>$svalue){
			if (isset($svalue['language'])){
				$first_key = $skey;
				$_SESSION[$first_key][$key] = $value;
			}
		}
	}
