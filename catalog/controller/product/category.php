...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			$parent = $this->model_catalog_category->getParentId($category_id);
			//print_r($parent);
			if($parent==0)
				$results = $this->model_catalog_category->getCategories($category_id);
			else
				$results = $this->model_catalog_category->getCategories($parent);
            ...
            $wishs = $this->model_account_wishlist->getWishlist();
            $wish=array();
            foreach($wishs as $w)
            {
                $wish[]=$w['product_id'];
	...
	$options= array();
			...
			$url = '';
			$data['path']=end(explode('_',$this->request->get['path']));
            ...
            $data['sorts'][] = array(
                'text'  => 'По скидке',
                'value' => 'p.prices-DESC',
                'href'  => $this->url->link('product/category', 'path=' . $this->request->get['path'] . '&sort=p.prices&order=DESC' . $url)
            );
