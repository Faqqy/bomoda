...
class ControllerBlogBlog extends Controller {
	...
	public function index() { 
		...
		$blog_cat = $this->model_blog_blog->getBlogCat($blog_id)['blog_category_id'];
        $cat_info = $this->model_blog_blog_category->getBlogCategory($blog_cat);

        $cats = $this->model_blog_blog_category->getBlogCategoriesByParentId($cat_info['parent_id']);

        foreach($cats as $cat)
        {
            if($cat!=$cat_info['parent_id']) {


                $cat_i = $this->model_blog_blog_category->getBlogCategory($cat);
                $items = $this->model_blog_blog->getBlogsByBlogCategoryId($cat);
                $bls=array();
                foreach($items as $item)
                {
                    $bls[]=array(
                        'name'=>$item['title'],
                        'href'=>$this->url->link('blog/blog', 'blogpath=' . $this->request->get['blogpath'] . '&blog_id=' . $item['blog_id'])
                    );
                }
                $data['cats'][] = array(
                    'id' => $cat,
		...
		if ($blog_info) {
            ...
            $data['article_id'] = $blog_info['blog_id'];
