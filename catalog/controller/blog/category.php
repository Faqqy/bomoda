...
class ControllerBlogCategory extends Controller {
	...
	if (isset($this->request->get['blogpath'])) {
		...
		if ($blog_category_info) {
            ...
            $cats = $this->model_blog_blog_category->getBlogCategoriesByParentId($blog_category_id);

            foreach($cats as $cat)
            {
                if($cat!=$blog_category_id) {


                    $cat_info = $this->model_blog_blog_category->getBlogCategory($cat);
                    $data['cats'][] = array(
                        'id' => $cat,
                        'name' => $cat_info['name']
                    );
            ...
            if($cats)
            {
                ...
                {
                    /*print_r($cat.' ');
                    print_r($blog_category_id.' ');*/
                    if($cat!=$blog_category_id) {
                        $results = $this->model_blog_blog->getBlogsByBlogCategoryId($cat, $pagefix, $limit);
                        $cat_info = $this->model_blog_blog_category->getBlogCategory($cat);
            ...
            else
            {
                ...
                foreach ($results as $result) {
                    ...
                    $data['blogs'][] = array(
                        ...
                        'category_name'=>$blog_category_info['name'],
                        'author' => $result['author'],
                        'comment_total' => $this->model_blog_blog->getTotalCommentsByBlogId($result['blog_id']),
                        'date_added_full' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
                        'description' => html_entity_decode($result['description']),
