...
class ControllerAccountSimpleEdit extends SimpleController {
    ...
    public function index($args = null) {
        ...
        if($this->customer->isLogged())
        {
            ...
            $cus=$this->model_account_customer->getCustomer($this->customer->isLogged());
            $this->_templateData['user']=$cus;
        ...
        //print_r($_GET);
        if($_GET['flag']=='pod')
            $this->_templateData['action'] = 'index.php?'.$this->simpleedit->getAdditionalParams().'route=account/simpleedit&flag=pod';
        else
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['submitted']) && $this->validate()) {
            ...
            $this->load->model('account/customer');
            $this->_templateData['change_ok'] = '1';
